...
const Crud = () => {
    ...
    const getBelanja = async () => {
        const responseBelanja = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dpa/search?tipe=Belanja DPA`, {withCredentials: true})
        ...
        if (belanja.namaFile && belanja.tahun) {
            ...
            if (belanja.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`Belanja-DPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dpa/${id}`, belanja, {withCredentials:true})
                    if (response.status === 200){
                        getBelanja()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Belanja DPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Belanja DPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    let urlBukti = '-'
                    ...
                    const response = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dpa", belanja, {withCredentials:true})
                    if (response.status === 201){
                        getBelanja()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Belanja DPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Belanja DPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deleteBelanja = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getBelanja()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Belanja DPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Belanja DPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data Belanja DPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data DPA Belanja</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={belanjaDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data Belanja DPA" modal className="p-fluid" footer={belanjaDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"Belanja DPA"} disabled />
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={deleteBelanjaDialog} blockScroll={true} closable={!confirmLoading} style={{ width: '450px' }} header="Konfirmasi" modal footer={deleteBelanjaDialogFooter} onHide={hideDeleteBelanjaDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {belanja && (
                                <span>
                                    Apakah anda yakin ingin menghapus data Belanja DPA <b>{belanja.namaFile}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
