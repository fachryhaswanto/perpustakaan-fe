...
const Crud = () => {
    ...
    const getHalPersetujuan = async () => {
        const responseHalPersetujuan = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dpa/search?tipe=Halaman Persetujuan DPA`, {withCredentials: true})
        ...
        if (halPersetujuan.namaFile && halPersetujuan.tahun) {
            ...
            if (halPersetujuan.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`Halaman-Persetujuan-DPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dpa/${id}`, halPersetujuan, {withCredentials:true})
                    if (response.status === 200){
                        getHalPersetujuan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Persetujuan DPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Persetujuan DPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    let urlBukti = '-'
                    ...
                    const response = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dpa", halPersetujuan, {withCredentials:true})
                    if (response.status === 201){
                        getHalPersetujuan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Persetujuan DPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    console.log(error)
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Persetujuan DPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deleteHalPersetujuan = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getHalPersetujuan()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Persetujuan DPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Persetujuan DPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data Halaman Persetujuan DPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data Halaman Persetujuan DPA</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={halPersetujuanDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data Halaman Persetujuan DPA" modal className="p-fluid" footer={halPersetujuanDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"Halaman Persetujuan DPA"} disabled />
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={deleteHalPersetujuanDialog} blockScroll={true} closable={!confirmLoading} style={{ width: '450px' }} header="Konfirmasi" modal footer={deleteHalPersetujuanDialogFooter} onHide={hideDeleteHalPersetujuanDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {halPersetujuan && (
                                <span>
                                    Apakah anda yakin ingin menghapus data Halaman Persetujuan DPA <b>{halPersetujuan.namaFile}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
