...
const Crud = () => {
    ...
    const getSkpd = async () => {
        const responseSkpd = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/search?tipe=SKPD DPPA`, {withCredentials: true})
        ...
        if (skpd.namaFile && skpd.tahun) {
            ...
            if (skpd.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`SKPD-DPPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/${id}`, skpd, {withCredentials:true})
                    if (response.status === 200){
                        getSkpd()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data SKPD DPPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data SKPD DPPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    ...
                    const response = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dppa", skpd, {withCredentials:true})
                    if (response.status === 201){
                        getSkpd()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data SKPD DPPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    console.log(error)
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data SKPD DPPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deleteSkpd = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getSkpd()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data SKPD DPPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data SKPD DPPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data SKPD DPPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data DPPA SKPD</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={skpdDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data SKPD DPPA" modal className="p-fluid" footer={skpdDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"SKPD DPPA"} disabled />
                        </div>
                        <div className="field">
