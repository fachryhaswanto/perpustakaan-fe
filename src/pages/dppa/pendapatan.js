...
const Crud = () => {
    ...
    const getPendapatan = async () => {
        const responsePendapatan = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/search?tipe=Pendapatan DPPA`, {withCredentials: true})
        ...
        if (pendapatan.namaFile && pendapatan.tahun) {
            ...
            if (pendapatan.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`Pendapatan-DPPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/${id}`, pendapatan, {withCredentials:true})
                    if (response.status === 200){
                        getPendapatan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Pendapatan DPPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Pendapatan DPPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    let urlBukti = '-'
                    ...
                    const responseUpload = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dppa", pendapatan, {withCredentials:true})
                    if (responseUpload.status === 201){
                        getPendapatan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Pendapatan DPPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Pendapatan DPPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deletePendapatan = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getPendapatan()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Pendapatan DPPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Pendapatan DPPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data Pendapatan DPPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data DPPA Pendapatan</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={pendapatanDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data Pendapatan DPPA" modal className="p-fluid" footer={pendapatanDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"Pendapatan DPPA"} disabled />
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={deletePendapatanDialog} blockScroll={true} closable={!confirmLoading} style={{ width: '450px' }} header="Konfirmasi" modal footer={deletePendapatanDialogFooter} onHide={hideDeletePendapatanDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {pendapatan && (
                                <span>
                                    Apakah anda yakin ingin menghapus data Pendapatan DPPA <b>{pendapatan.namaFile}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
