...
const Crud = () => {
    ...
    const getHalDepan = async () => {
        const responseHalDepan = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/search?tipe=Halaman Depan DPPA`, {withCredentials: true})
        ...
        if (halDepan.namaFile && halDepan.tahun) {
            ...
            if (halDepan.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`Halaman-Depan-DPPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/${id}`, halDepan, {withCredentials:true})
                    if (response.status === 200){
                        getHalDepan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Depan DPPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Depan DPPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    let urlBukti = '-'
                    ...
                    const response = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dppa", halDepan, {withCredentials:true})
                    if (response.status === 201){
                        getHalDepan()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Depan DPPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Depan DPPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deleteHalDepan = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getHalDepan()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Halaman Depan DPPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Halaman Depan DPPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data Halaman Depan DPPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data Halaman Depan DPPA</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={halDepanDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data Halaman Depan DPPA" modal className="p-fluid" footer={halDepanDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"Halaman Depan DPPA"} disabled />
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={deleteHalDepanDialog} blockScroll={true} closable={!confirmLoading} style={{ width: '450px' }} header="Konfirmasi" modal footer={deleteHalDepanDialogFooter} onHide={hideDeleteHalDepanDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {halDepan && (
                                <span>
                                    Apakah anda yakin ingin menghapus data Halaman Depan DPPA <b>{halDepan.namaFile}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
