...
const Crud = () => {
    ...
    const getRincBelanja = async () => {
        const responseRincBelanja = await axios.get(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/search?tipe=Rincian Belanja DPPA`, {withCredentials: true})
        ...
        if (rincBelanja.namaFile && rincBelanja.tahun) {
            ...
            if (rincBelanja.id) {
                ...
                try {
                    ...
                    if (fileUpload) {
                        const {url, fields, filePath} = await getBucketURL(`Rincian-Belanja-DPPA/${session.username}`, fileUpload)
                    ...
                    const response = await axios.patch(process.env.NEXT_PUBLIC_BASE_URL_API + `/dppa/${id}`, rincBelanja, {withCredentials:true})
                    if (response.status === 200){
                        getRincBelanja()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Rincian Belanja DPPA Berhasil Diperbarui', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Rincian Belanja DPPA Gagal Diperbarui', life: 3000 });
                }                              
            } else {
                try {
                    let urlBukti = '-'
                    ...
                    const responseUpload = await axios.post(process.env.NEXT_PUBLIC_BASE_URL_API + "/dppa", rincBelanja, {withCredentials:true})
                    if (responseUpload.status === 201){
                        getRincBelanja()
                        toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Rincian Belanja DPPA Berhasil Disimpan', life: 3000 });
                    }
                } catch (error) {
                    toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Rincian Belanja DPPA Gagal Disimpan', life: 3000 });
                }               
            }
    ...
    const deleteRincBelanja = async () => {
        ...
        try {
            ...
            if (response.status === 200){
                getRincBelanja()
                toast.current.show({ severity: 'success', summary: 'Sukses', detail: 'Data Rincian Belanja DPPA Berhasil Dihapus', life: 3000 });
            }
        } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Kesalahan', detail: 'Data Rincian Belanja DPPA Gagal Dihapus', life: 3000 });
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Tambah Data Rincian Belanja DPPA" icon="pi pi-plus" className="p-button-primary p-button-raised mr-2" onClick={openNew} disabled={disabledTambah} />
                </div>
            </React.Fragment>
        );
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Data DPPA Rincian Belanja</h5>
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={rincBelanjaDialog} blockScroll={true} closable={!simpanLoading} style={{ width: '450px' }} header="Data Rincian Belanja DPPA" modal className="p-fluid" footer={rincianBelanjaDialogFooter} onHide={hideDialog}>                        
                        <div className="field">
                        ...
                        <div className="field">
                            ...
                            <InputText id="tipe" value={"Rincian Belanja DPPA"} disabled />
                        </div>
                        <div className="field">
                    ...
                    <Dialog visible={deleteRincBelanjaDialog} blockScroll={true} closable={!confirmLoading} style={{ width: '450px' }} header="Konfirmasi" modal footer={deleteRincianBelanjaDialogFooter} onHide={hideDeleteRincBelanjaDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {rincBelanja && (
                                <span>
                                    Apakah anda yakin ingin menghapus data Rincian Belanja DPPA <b>{rincBelanja.namaFile}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
